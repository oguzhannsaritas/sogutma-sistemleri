...
const SLIDES: Slide[] = [
    { title1: "Hijyenik Depolama", image: "/1.jpg", alt: "Hijyenik Soğuk Depolama Alanı" },
    { title1: "Hızlı Soğutma", image: "/2.jpg", alt: "Hızlı Soğutma Sistemi" },
    { title1: "Endüstriyel Güvenilirlik", image: "/3.jpg", alt: "Endüstriyel Soğutma Ünitesi" },
    { title1: "Enerji Verimli Çözüm", image: "/4.jpg", alt: "Enerji Tasarruflu Soğutma" },
    { title1: "Her Zaman Taze", image: "/5.jpg", alt: "Soğuk Oda Ünitesi" },
    { title1: "Akıllı Nem Kontrolü", image: "/6.webp", alt: "Nem & Sıcaklık Takibi" },
    { title1: "Kesintisiz Soğuk Zincir", image: "/7.png", alt: "Endüstriyel Soğuk Zincir" },
]
...
export function HeroSection() {
    ...
    const t0Ref = useRef<number>(performance.now()) // ⟵ sayaç başlangıcı
    const touchX = useRef<number | null>(null)
    const sectionRef = useRef<HTMLElement | null>(null)
    const wasInView = useRef(false)

    // Autoplay loop (tek instance)
    const startAutoplay = () => {
        if (raf.current) {
        ...
        const loop = (t: number) => {
            if (t - t0Ref.current >= AUTOPLAY_MS) {
                // autoplay ile sonraki slayta geç
                setIndex((p) => (p + 1) % SLIDES.length)
                // yeni döngü için başlangıcı bu frame'e sabitle
                t0Ref.current = t
            }
    ...
    // Görünürlük kontrolü: görünürken çalış, değilken dur
    useEffect(() => {
    ...
    // İlk açılışta autoplay’i başlat (görünürlük hook’u da güvenli)
    useEffect(() => {
        startAutoplay()
        return () => {
    ...
    // Kullanıcı etkileşimi olduğunda sayacı sıfırla
    const resetAutoplayCountdown = () => {
        t0Ref.current = performance.now()
        // görünürlük yüzünden RAF durmuşsa tekrar başlat
        if (!raf.current) startAutoplay()
    }
    ...
    return (
        ...
        >
            ...
            <div className="container mx-auto px-4 h-full relative z-10">
                <div className="grid md:grid-cols-2 gap-8 h-full items-center -mt-14">
                    ...
                    <div className="flex flex-col items-center">
                        <div className={`relative h-[260px] sm:h-[360px] md:h-[500px] lg:h-[600px] w-full img-stage ${variant} overflow-hidden rounded-lg`}>
            ...
            <style jsx>{`
                ...
                @keyframes diagIn {
                    0% { opacity: 0; transform: translateX(24px) scale(1.02); clip-path: polygon(100% 0,100% 0,100% 100%,80% 100%); }
                    100% { opacity: 1; transform: translateX(0) scale(1); clip-path: polygon(0 0,100% 0,100% 100%,0 100%); }
                }
                @keyframes diagOut {
                ...
                @keyframes splitIn {
                    0% { opacity: 0; clip-path: polygon(50% 0,50% 0,50% 100%,50% 100%); }
                    100% { opacity: 1; clip-path: polygon(0 0,100% 0,100% 100%,0 100%); }
                }
                @keyframes splitOut {
                    0% { opacity: 1; }
                    100% { opacity: 0; clip-path: polygon(50% 0,100% 0,100% 100%,50% 100%); }
                }

                @keyframes kenBurns {
                    0% { transform: scale(1.02) translate3d(6px,0,0); }
